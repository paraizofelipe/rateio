...
**Objetivo:** `docs/appwrite-setup.md` permitindo criar manualmente projeto/database/collections/atributos/índices/permissões em `YOUR_APPWRITE_HOST/v1`, e o app conectar sem ajuste.
**Conteúdo obrigatório:**
- Endpoint `https://YOUR_APPWRITE_ENDPOINT/v1`; onde obter ProjectID, DatabaseID e os 3 CollectionIDs.
...
**Já existe:** projeto Appwrite `cost` (ID `YOUR_PROJECT_ID`) e plataforma Android registrada. T2 foca em: criar **Database**, as **3 collections** (atributos/índices), habilitar **Email/Password** e ajustar **permissões**.
...
**Valores reais (confirmados):** SDK `io.appwrite:sdk-for-android:7.0.1`; ENDPOINT `https://YOUR_APPWRITE_ENDPOINT/v1`; PROJECT_ID `YOUR_PROJECT_ID`; DATABASE_ID `YOUR_DATABASE_ID`; COLLECTION_GRUPOS `YOUR_COLLECTION_GRUPOS`; COLLECTION_PESSOAS `YOUR_COLLECTION_PESSOAS`; COLLECTION_GASTOS `YOUR_COLLECTION_GASTOS`. Package do app = `dev.paraizo.cost` (igual à plataforma Appwrite). Certificado é válido (Google Trust Services) → **NÃO** usar `setSelfSigned(true)`. Init:
```kotlin
...
2. **Appwrite SDK (T1/T4)** — ✅ resolvido: `io.appwrite:sdk-for-android:7.0.1`, projectId `YOUR_PROJECT_ID`, endpoint `https://YOUR_APPWRITE_ENDPOINT/v1`, cert válido (sem `setSelfSigned`). Resta validar a API concreta da 7.x na implementação.
